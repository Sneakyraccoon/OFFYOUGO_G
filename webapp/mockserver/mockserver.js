sap.ui.define(["sap/ui/core/util/MockServer"], (MockServer) => {
    ...
    return {
        init: () => {
            ...
            const aRequests = []; // Массив заявок
            const aInboxes = [];  // Инбокс менеджеров
            ...
            oMockServer.setRequests([
                ...
                },
                {
                    ...
                    response: (oXhr) => {
                        const oData = JSON.parse(oXhr.requestBody);
                        const requestId = aRequests.length > 0 ? aRequests[aRequests.length - 1].id + 1 : 1;
                        aRequests.push({ ...oData, id: requestId });
